...
export class MainOracleAgent {
    ...
    dreamAgent = new DreamAgent();
    shadowAgent = new ShadowAgent();
    ...
    parseStoryRequest(query) {
        const storyKeywords = [
            "tell me a story",
            "share a story",
            "create a story",
        ];
        const isStoryRequest = storyKeywords.some((keyword) => query.toLowerCase().includes(keyword));
        if (!isStoryRequest)
            return null;
        const elementalThemes = ["fire", "water", "earth", "air", "aether"];
        const elementalTheme = elementalThemes.find((theme) => query.toLowerCase().includes(theme)) ||
            "aether";
        return {
