...
export async function updatePersonalityWeights(adjustments) {
  ...
  const { error } = await supabase.from("personality_weights").upsert(
    adjustments.map((adj) => ({
      element: adj.element,
      weight: adj.weight,
  ...
  console.log("Updated personality weights:", adjustments); // Log the updated weights
}
export async function getPersonalityWeights() {
  ...
  console.log("Fetched personality weights:", data); // Log the fetched data
  return data.map((row) => ({
    element: row.element,
    weight: row.weight,
...
// Testing function for interaction with Supabase and logs
async function testMonitoringService() {
  try {
    // Test getPersonalityWeights function
    const weights = await getPersonalityWeights();
    console.log("Current personality weights:", weights);
  } catch (err) {
    console.error("Error testing getPersonalityWeights:", err);
  }
  try {
    // Test updatePersonalityWeights function with mock data
    const adjustments = [
      { element: "fire", weight: 0.9, confidence: 0.8 },
      { element: "water", weight: 0.7, confidence: 0.6 },
    ];
    await updatePersonalityWeights(adjustments);
    console.log("Successfully updated personality weights");
  } catch (err) {
    console.error("Error testing updatePersonalityWeights:", err);
  }
}
