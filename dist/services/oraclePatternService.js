...
export async function updatePatternFrequency(patternId) {
...
export async function getPatternsByType(patternType, minConfidence = 0.7) {
  const { data, error } = await supabase
    .from("oracle_patterns")
    .select("*")
...
export async function getTopPatterns(limit = 10, minConfidence = 0.7) {
  const { data, error } = await supabase
    .from("oracle_patterns")
    .select("*")
    .gte("confidence", minConfidence)
