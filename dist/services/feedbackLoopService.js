...
export async function recordFeedback(userId, responseId, feedback) {
    try {
...
export async function analyzeFeedbackTrends(timeframe) {
    try {
        ...
        const elementalEffectiveness = feedback.reduce((acc, f) => {
            if (f.element) {
                if (!acc[f.element]) {
        ...
        const patternEffectiveness = feedback.reduce((acc, f) => {
        ...
        const { data: analysis, error: analysisError } = await supabase
            ...
            elemental_effectiveness: Object.fromEntries(Object.entries(elementalEffectiveness).map(([k, v]) => [
                k,
                v.total / v.count,
            ])),
            pattern_effectiveness: Object.fromEntries(Object.entries(patternEffectiveness).map(([k, v]) => [
                k,
                v.total / v.count,
            ])),
        })
...
async function applyLearningAdjustments(analysisId) {
    try {
        ...
        const elementalAdjustments = Object.entries(analysis.elemental_effectiveness).map(([element, effectiveness]) => ({
