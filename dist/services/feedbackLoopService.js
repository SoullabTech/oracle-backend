...
async function applyLearningAdjustments(analysisId) {
  try {
    ...
    const { data: adjustments, error: adjustmentError } = await supabase
      ...
      .select();
    if (adjustmentError || !adjustments) throw adjustmentError;
    const trackingEntries = adjustments.map((adjustment) => ({
