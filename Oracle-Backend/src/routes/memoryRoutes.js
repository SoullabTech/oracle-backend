"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g = Object.create((typeof Iterator === "function" ? Iterator : Object).prototype);
    return g.next = verb(0), g["throw"] = verb(1), g["return"] = verb(2), typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (g && (g = 0, op[0] && (_ = 0)), _) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
Object.defineProperty(exports, "__esModule", { value: true });
var express_1 = require("express");
var multer_1 = require("multer");
var memoryController_js_1 = require("../controllers/memoryController.js");
var router = express_1.default.Router();
var upload = (0, multer_1.default)({ dest: 'uploads/' });
router.post('/upload', upload.single('file'), function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
    var result, result, error_1;
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0:
                _a.trys.push([0, 6, , 7]);
                if (!req.file) return [3 /*break*/, 2];
                return [4 /*yield*/, (0, memoryController_js_1.storeMemory)({
                        id: "mem-".concat(Date.now()),
                        content: req.file.path,
                        type: 'audio',
                        timestamp: Date.now(),
                        clientId: req.body.clientId || 'unknown'
                    })];
            case 1:
                result = _a.sent();
                res.json({ memory_id: result.id, message: 'Audio uploaded successfully' });
                return [3 /*break*/, 5];
            case 2:
                if (!req.body.content) return [3 /*break*/, 4];
                return [4 /*yield*/, (0, memoryController_js_1.storeMemory)({
                        id: "mem-".concat(Date.now()),
                        content: req.body.content,
                        type: 'text',
                        timestamp: Date.now(),
                        clientId: req.body.clientId || 'unknown'
                    })];
            case 3:
                result = _a.sent();
                res.json({ memory_id: result.id, message: 'Text uploaded successfully' });
                return [3 /*break*/, 5];
            case 4:
                res.status(400).json({ error: 'No content provided' });
                _a.label = 5;
            case 5: return [3 /*break*/, 7];
            case 6:
                error_1 = _a.sent();
                console.error("Error in /upload:", error_1);
                res.status(500).json({ error: 'Server error uploading memory' });
                return [3 /*break*/, 7];
            case 7: return [2 /*return*/];
        }
    });
}); });
router.get('/insights', function (_req, res) { return __awaiter(void 0, void 0, void 0, function () {
    var insights, error_2;
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0:
                _a.trys.push([0, 2, , 3]);
                return [4 /*yield*/, (0, memoryController_js_1.getMemoryInsights)()];
            case 1:
                insights = _a.sent();
                res.json({ insights: insights });
                return [3 /*break*/, 3];
            case 2:
                error_2 = _a.sent();
                console.error("Error in /insights:", error_2);
                res.status(500).json({ error: 'Server error retrieving insights' });
                return [3 /*break*/, 3];
            case 3: return [2 /*return*/];
        }
    });
}); });
exports.default = router;
