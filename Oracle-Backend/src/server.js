"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g = Object.create((typeof Iterator === "function" ? Iterator : Object).prototype);
    return g.next = verb(0), g["throw"] = verb(1), g["return"] = verb(2), typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (g && (g = 0, op[0] && (_ = 0)), _) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
Object.defineProperty(exports, "__esModule", { value: true });
var express_1 = require("express");
var dotenv_1 = require("dotenv");
var cors_1 = require("cors");
var orchestrator_js_1 = require("./core/orchestrator.js");
var authRoutes_js_1 = require("./routes/authRoutes.js");
var chatRoutes_js_1 = require("./routes/chatRoutes.js");
var memoryRoutes_js_1 = require("./routes/memoryRoutes.js");
var facilitatorRoutes_js_1 = require("./routes/facilitatorRoutes.js");
dotenv_1.default.config();
var app = (0, express_1.default)();
var PORT = process.env.PORT || 5001;
if (!process.env.PORT) {
    console.error('Error: PORT environment variable is not set.');
    process.exit(1); // Exit the process if the port is not set.
}
app.use((0, cors_1.default)());
app.use(express_1.default.json());
// Endpoint to test if the backend is live
app.get('/api/ping', function (_req, res) {
    res.json({ message: 'Backend is live' });
});
// Auth routes
app.use('/api/auth', authRoutes_js_1.default);
// Chat routes
app.use('/api/chat', chatRoutes_js_1.default);
// Memory routes
app.use('/api/memory', memoryRoutes_js_1.default);
// Facilitator routes
app.use('/api/facilitator', facilitatorRoutes_js_1.default);
// Generate prompt for LangChain
app.post('/api/generate-prompt', function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
    var _a, query, userId, result, error_1;
    return __generator(this, function (_b) {
        switch (_b.label) {
            case 0:
                _a = req.body, query = _a.query, userId = _a.userId;
                if (!query || !userId) {
                    res.status(400).json({ error: 'Missing query or userId' });
                    return [2 /*return*/];
                }
                _b.label = 1;
            case 1:
                _b.trys.push([1, 3, , 4]);
                return [4 /*yield*/, (0, orchestrator_js_1.runLangChain)(query)];
            case 2:
                result = _b.sent();
                res.json({ prompt: result });
                return [3 /*break*/, 4];
            case 3:
                error_1 = _b.sent();
                console.error('Error processing LangChain query:', error_1);
                res.status(500).json({ error: 'Error processing LangChain query. Please try again later.' });
                return [3 /*break*/, 4];
            case 4: return [2 /*return*/];
        }
    });
}); });
// Trigger Prefect Flow
app.post('/api/trigger-flow', function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
    var payload, result, error_2;
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0:
                payload = req.body;
                _a.label = 1;
            case 1:
                _a.trys.push([1, 3, , 4]);
                return [4 /*yield*/, (0, orchestrator_js_1.triggerPrefectFlow)(payload)];
            case 2:
                result = _a.sent();
                res.json(result);
                return [3 /*break*/, 4];
            case 3:
                error_2 = _a.sent();
                console.error('Error triggering Prefect flow:', error_2);
                res.status(500).json({ error: 'Error triggering Prefect flow. Please try again later.' });
                return [3 /*break*/, 4];
            case 4: return [2 /*return*/];
        }
    });
}); });
app.listen(PORT, function () {
    console.log("\uD83D\uDE80 Server is running on port ".concat(PORT));
});
