...
import TarotCardOfTheDay from "@/components/TarotCardOfTheDay";
...
export default function CollectiveDeckPulse() {
  ...
  useEffect(() => {
    async function load() {
      ...
      const tarotMentions: Record<string, number> = {};
      ...
      for (const entry of entries) {
        ...
        for (const card of Object.keys(TarotDeck)) {
          if (content.includes(card.toLowerCase())) {
            ...
            if (trends[card][dayIndex] !== undefined) trends[card][dayIndex] += 1;
  ...
  return (
    <div className="p-6 space-y-6">
      <PulseSummary
        tone={tone}
        resonanceOptIn={resonanceOptIn}
        onToggleResonance={() => setResonanceOptIn(!resonanceOptIn)}
        badgeNotice={badgeNotice}
        filter={filter}
        onFilterChange={setFilter}
      />

      <TrendingCards cards={topCards} router={router} tooltipComponent={TooltipCardInsights} />

      {topCards.length > 0 && <PulseChart trendData={trendData} topCards={topCards} filter={filter} />}

      {ritualSuggestion && <SuggestedRitual ritual={ritualSuggestion} />}

      <CollectiveExportPanel
        enabled={shareSummary}
        onToggle={() => setShareSummary(!shareSummary)}
        topCard={topCards[0]}
        ritual={ritualSuggestion}
      />

      <PulseArchiveViewer entries={archive} />
    </div>
  );
}
