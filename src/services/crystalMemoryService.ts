...
let memoryStore: any[] = []; // In-memory fallback (replace with DB or Supabase later)

export async function saveToCrystalMemory(entry: Record<string, any>) {
  try {
...
export async function getCrystalMemory(filter: Partial<Record<string, any>> = {}) {
  try {
