...
export async function analyzeFeedback(timeframe: {
  ...
  try {
    ...
    const elementalScores = feedback.reduce(
      (acc, f) => {
    ...
    const patterns = feedback.reduce(
      (acc, f) => {
    ...
    const patternEffectiveness = Object.entries(patterns).reduce(
...
export async function updatePersonalityWeights(
  ...
  try {
    ...
    const newWeights = Object.entries(analysis.elementalEffectiveness).map(
      ([element, effectiveness]) => {
        const currentWeight =
          currentWeights?.find((w) => w.element === element)?.weight || 0.5;
        // Adjust weight based on effectiveness with dampening
        const adjustment = (effectiveness - 0.5) * 0.1; // 10% max adjustment
        const newWeight = Math.max(
          0.1,
          Math.min(0.9, currentWeight + adjustment),
        );
