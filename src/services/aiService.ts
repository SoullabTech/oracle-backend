...
function calculateQueryComplexity(text: string): number {
  ...
  complexity += (text.match(/\b(why|how|what if|compare|analyze)\b/gi) || []).length * 0.3; // Analytical terms

  return Math.min(Math.max(complexity, 0), 1);
}

function determineQueryType(text: string, profile: ElementalProfile): RoutingCriteria['queryType'] {
  ...
  const bestMatch = weightedScores.reduce((prev, current) =>
    current.score > prev.score ? current : prev
  );

  return bestMatch.type as RoutingCriteria['queryType'];
}