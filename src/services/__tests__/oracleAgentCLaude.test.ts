...
describe("OracleAgent Claude Path Tests", () => {
  ...
  it("should return a valid response from Claude-3", async () => {
    ...
    const mockedAnthropic = (Anthropic as unknown as vi.Mock).mock.results[0]
      .value;
  ...
  it("should throw an error if Claude-3 request fails", async () => {
    const mockedAnthropic = (Anthropic as unknown as vi.Mock).mock.results[0]
      .value;
    mockedAnthropic.messages.create.mockRejectedValueOnce(
      new Error("Claude down"),
    );

    await expect(oracleAgent.performTask("Broken Claude")).rejects.toThrow(
      "OracleAgent error: Claude down",
    );
  });
});
