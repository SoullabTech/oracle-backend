 ...
 */
const facetKeywords: Record<string, FacetMapping['facet']> = {
  warmth: 'fire.warmth',
  passion: 'fire.warmth',
  flow: 'water.emotion',
  calm: 'water.emotion',
 ...
 */
export function detectFacetFromInput(text: string): FacetMapping['facet'] | null {
 ...
 */
export async function getAllFacetMappings(): Promise<FacetMapping[]> {
  try {
    const { data, error } = await supabase
      .from<FacetMapping>('facet_mappings')
      .select('*')
