...
app.get('/healthz', async (_req, res) => {
  try {
    const { error } = await supabase.from('insight_history').select('1').limit(1);
    if (error) return res.status(503).send('Database error');
...
try {
  ...
  console.warn('⚠️ Could not load Swagger YAML:', err);
}
