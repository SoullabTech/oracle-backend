...
let posts: any[] = []; // Temporary in-memory storage (replace with DB later)
...
router.post('/post', (req, res) => {
  try {
    ...
    if (!title || !symbol) throw new ValidationError('Title and symbol are required');
...
router.get('/posts', (req, res) => {
  ...
  if (element) filtered = filtered.filter((p) => p.element === element);
