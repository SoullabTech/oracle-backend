/*
  # Create Survey and Elemental Profile Tables

  1. New Tables
    - survey_questions
      - id (uuid, primary key)
      - text (text)
      - element (text, constrained to valid elements)
      - weight (integer)
    ...
    - elemental_profiles
      ...
      - crystal_focus (jsonb, structured data for focus area)
      - created_at (timestamptz)
  ...
  3. Initial Data
    - Insert starter survey questions
*/

-- Drop existing tables if they exist (clean slate)
DROP TABLE IF EXISTS survey_responses;
DROP TABLE IF EXISTS survey_questions;
DROP TABLE IF EXISTS elemental_profiles;
...
CREATE TABLE elemental_profiles (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id uuid REFERENCES auth.users(id) ON DELETE CASCADE UNIQUE,
  fire integer NOT NULL CHECK (fire BETWEEN 0 AND 100) DEFAULT 50,
  water integer NOT NULL CHECK (water BETWEEN 0 AND 100) DEFAULT 50,
  earth integer NOT NULL CHECK (earth BETWEEN 0 AND 100) DEFAULT 50,
  air integer NOT NULL CHECK (air BETWEEN 0 AND 100) DEFAULT 50,
  aether integer NOT NULL CHECK (aether BETWEEN 0 AND 100) DEFAULT 50,
