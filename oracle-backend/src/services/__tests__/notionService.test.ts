...
describe('Notion Service', () => {
  ...
  describe('fetchNotionPages', () => {
    it('should fetch and transform pages', async () => {
      ...
      (Client.prototype.databases.query as jest.Mock).mockResolvedValue({
        results: mockPages,
      });

      (Client.prototype.blocks.children.list as jest.Mock).mockResolvedValue(mockBlocks);
    ...
    it('should handle fetch errors', async () => {
      (Client.prototype.databases.query as jest.Mock).mockRejectedValue(
        new Error('API error')
      );
